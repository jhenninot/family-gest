...
- `server/alexa/` — private Alexa skill (voice: add events, shopping items, dishes, absences/presences, guests). `POST /api/alexa/:familySlug/:token`, token per family (`AlexaConnector`, managed from family settings like the MCP connector). Requests are verified with Amazon's signature on the **raw body** (`req.rawBody`, captured by `express.json`'s `verify` for `/api/alexa/` only) before anything else. `handlers.js` (dialogue) only talks to `actions.js` (DB writes reusing `createEventOrSeries`, `upsertAbsenceRecord`, `createMealGuestsBatch`…), so it can be tested with a fake API. The fr-FR interaction model is generated by `interactionModel.js` (downloadable from family settings, with the members' first names). `sync.js` optionally pushes that model to Amazon through SMAPI (Login with Amazon OAuth: client ID/secret + refresh token stored encrypted on `AlexaConnector.sync`, callback `GET /api/alexa-oauth/callback`), when its hash changes — checked at startup and every 10 minutes. User guide: `ALEXA.md` (also in-app: `src/components/AlexaSetupGuide.vue`).
